...
# Indicar directorio
dir <- "C:/Users/ProfeJLJara/Downloads"
...
p <- ggscatter(datos, x = "Height", y = "Weight")
print(p)
...
# Calculamos su significancia (H0 sin dirección):
...
# Esto es "Pearson's product-moment correlation"
# Condiciones: distrib. muestral es aprox. normal
# - si los datos siguen aprox. una distrib. normal
# - tenemos una muestra grande
# Pero existen dos métodos no paramétricos como alternativa;
# - Spearman's rho: r con el ranking de los datos
# - kendall's tau: ajuste que mejora problemas con empates
cat("\n\n")
...
p2 <- ggscatter(datos, x = "Height", y = "Weight"
          , add = "reg.line"
          , conf.int = TRUE
          , add.params = list(color = "red", fill = "lightgray")
          )
p2 <- p2 + stat_cor(method = "pearson", label.x = 155, label.y = 90)
print(p2)

# Aquí hay otra interpretación de R^2
# Con el modelo que hemos trabajo, i.e. la media
